...
- Scenario L1 (discovery success): Given `lefthook.yml` declares gitvault integration and adapter is available, when integration check runs, then adapter is selected deterministically.
...
- Scenario L3 (missing adapter in automation): Given `lefthook` config exists but adapter is missing, when run in CI/`--no-prompt`, then failure contract is deterministic with install guidance.
